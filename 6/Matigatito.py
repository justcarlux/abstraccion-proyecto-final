'''
NOTA: CAMBIAR RUTAS DE LAS IMAGENES, Y SONIDOS
'''
import pygame
import random
import os
...
pygame.display.set_caption("Matigatito")
...
ruta_img = os.path.join(os.path.dirname(__file__), 'imagenes')
...
try:
    gato_frames = [cargar_img(f"gato{i}.png", (400, 400)).convert_alpha() for i in range(1, 5)]
    gato_img_estatica = cargar_img("gato.png", (400, 400)).convert_alpha() # Para pantalla de bienvenida y nube de pensamiento

    papa = cargar_img("papa.png", (140,140)).convert_alpha()
    fresa = cargar_img("fresa.png", (140,140)).convert_alpha()
    pimenton = cargar_img("pimenton.png", (140,140)).convert_alpha()
    dona = cargar_img("dona.png",(140,140)).convert_alpha()
    uvas = cargar_img("uvas.png", (140,140)).convert_alpha()
    pera = cargar_img("pera.png", (140,140)).convert_alpha()
    manzana = cargar_img("manzana.png", (140,140)).convert_alpha()
    limones = cargar_img("limones.png", (140,140)).convert_alpha()
    torta = cargar_img("torta.png", (140,140)).convert_alpha()
    berenjena = cargar_img("berenjena.png", (140,140)).convert_alpha()
    bananas= cargar_img("bananas.png", (140,140)).convert_alpha()
    calabaza= cargar_img("calabaza.png", (140,140)).convert_alpha()
    
    pensamiento = cargar_img("pensamiento.png", (430, 290)).convert_alpha()
    fondo_juego = cargar_img("fondo.png", (ancho, alto)).convert_alpha()
    fondo_bienvenida = cargar_img("fondo_bienvenida.png", (ancho, alto)).convert_alpha()
    letras_bienvenida = cargar_img("letras_bienvenida.png", (500, 300)).convert_alpha()
    info = cargar_img("info.png", (750, 500)).convert_alpha()
...
fuente_ruta = os.path.join(os.path.dirname(__file__), 'fuentes')
fuente_principal = pygame.font.Font(os.path.join(fuente_ruta, "AGAALER.TTF"), 60)
fuente_comentarios = pygame.font.Font(os.path.join(fuente_ruta, "AGAALER.TTF"), 56)
...
def dibujar_escena_juego(gato_piensa_item, indice_arrastrado=None, pos_arrastre=None):
    ventana.blit(fondo_juego, (0, 0))
    ventana.blit(pensamiento, (520, 20)) # Nube de pensamiento

    # Imagen que el gato está pensando
    img_pensada = gato_piensa_item.get('img')
    if img_pensada:
        rect_img_pensada = img_pensada.get_rect(center=(720, 130))
    ...
    if indice_arrastrado is not None and pos_arrastre is not None:
        rect_arrastre_fondo = pygame.Rect(*pos_arrastre, 160, 160)  # Usa tus dimensiones reales
    ...
    pygame.draw.rect(ventana, color_fondo_principal, boton_anterior_rect, border_radius=10)
    pygame.draw.rect(ventana, color_borde, boton_anterior_rect, 2, border_radius=10)
    pygame.draw.rect(ventana, color_fondo_principal, boton_siguiente_rect, border_radius=10)
    pygame.draw.rect(ventana, color_borde, boton_siguiente_rect, 2, border_radius=10)
...
def main_nivel_1():
    ...
    gato_piensa_item = random.choice(opciones_alimentos) # El item que el gato está pensando
    mensaje_feedback = ""
    ...
    # Inicializar el sprite del gato animado y su grupo
    ...
    while juego_corriendo:
        ...
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                ...
                if pygame.time.get_ticks() - timer_feedback > duracion_feedback: # Solo permite clics después del feedback
                    ...
                    if boton_anterior_rect.collidepoint(evento.pos):
                        ...
                        # Clic en opciones de alimentos
                        opciones_visibles, rects_opciones_visibles = obtener_opciones_y_rects_visibles()
                        for idx, rect in enumerate(rects_opciones_visibles):
                            if rect.collidepoint(evento.pos):
                                arrastrando_item = True
                                idx_arrastrado = idx # Índice dentro de las opciones visibles
                ...
                if arrastrando_item:
                    ...
                    rect_soltar = pygame.Rect(*pos_soltar, (100,100)[0], (100,100)[1])
                    if rect_bandeja.colliderect(rect_soltar): # Si se soltó sobre la bandeja
                        opciones_visibles, _ = obtener_opciones_y_rects_visibles()
                        # Obtener la opción real que fue arrastrada
                        ...
                        if num_aciertos + num_errores == 6: # Si se han realizado 6 intentos
                            finalizar_juego(num_aciertos, num_errores)
                            timer_feedback = pygame.time.get_ticks()
                            juego_corriendo = False  # Termina el juego

                        gato_piensa_item = random.choice(opciones_alimentos) # Nuevo item para el gato
                        mezclar_opciones_juego() # Volver a mezclar opciones
        ...
        ventana.blit(fondo_juego, (0, 0))
        dibujar_escena_juego(gato_piensa_item, idx_arrastrado, pos_arrastre)
        grupo_sprites_gato.update(dt) # Actualiza la animación del gato
        grupo_sprites_gato.draw(ventana) # Dibuja el gato
        
        if mensaje_feedback:
            texto_feedback = fuente_comentarios.render(mensaje_feedback, True, color_feedback)
            # Mostrar el mensaje en la esquina superior izquierda
...
def main_nivel_2():
    ...
    while juego_activo:
        ...
        ventana.blit(pensamiento, (520, 20))  # Nube en la parte superior
        ...
        rect_texto_suma_nube = texto_suma_nube.get_rect(center=(720, 130))  # Centrar en la nube de pensamiento
    ...
    while en_niveles:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                ...
                pos_mouse = evento.pos
                if rect_boton_nivel1.collidepoint(pos_mouse):
                    pantalla_bienvenida() # Regresar a la bienvenida antes de iniciar el juego
                    main_nivel_1() # Iniciar el juego principal
                elif rect_boton_nivel2.collidepoint(pos_mouse):
                    pantalla_bienvenida()
                    siguiente()
                    main_nivel_2()
                    # Lógica para el Nivel 2 (aún no implementada)
