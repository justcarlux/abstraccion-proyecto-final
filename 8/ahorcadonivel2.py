...
from palabras import PALABRAS_NIVEL2  # type: ignore

class AhorcadoNivel2:
    def __init__(self, fondo, aleatorio=True):
        ...
        self.palabras = random.sample(PALABRAS_NIVEL2, self.max_rondas) if aleatorio else PALABRAS_NIVEL2[:self.max_rondas]
            ...
            if self.mostrar_opciones:
                ...
                if self.rect_opcion_pista.collidepoint(pos):
                    if self.monedas >= 2 and self.pistas_restantes > 0:
                        self.sonido_boton.play()  # Sonido usar pista
